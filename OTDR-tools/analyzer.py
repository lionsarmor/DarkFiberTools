...
def parse_sor_files():
    ...
    if not os.path.isfile(rbOTDR_path) or not os.access(rbOTDR_path, os.X_OK):
        messagebox.showerror("Error", "Invalid rbOTDR path. Please ensure it exists and is executable.")
        return
        ...
        for root, _, files in os.walk(input_folder):
            for file in files:
                if file.lower().endswith('.sor'):
                    ...
                    try:
                        # Pass the input file and output folder to the Ruby script
                        subprocess.run([rbOTDR_path, input_file_path, output_folder], check=True)
